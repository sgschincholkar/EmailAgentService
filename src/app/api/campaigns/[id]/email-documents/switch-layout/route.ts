...
import { SwitchLayoutRequestSchema } from "@/generation/switch-layout-schema";
import { switchEmailDocumentLayout } from "@/generation/switch-email-document-layout";
...
export async function POST(
  ...
  const parsed = SwitchLayoutRequestSchema.safeParse(body);
  if (!parsed.success) {
  ...
  try {
    const document = await switchEmailDocumentLayout(
      campaignId,
      parsed.data.baseDocumentId,
      parsed.data.expectedVersion,
      parsed.data.targetLayoutId,
    );
    return NextResponse.json(
